...
root_merge_gnps <- read_excel("RL_merged_gnps_09102020.xlsx")
...
#read in the standards file, curated manually by comparing rt and mz to authentic standards
standards <- read_excel("standard_IDs_03232022.xlsx")

var_match <- rbind(var_name_pos, var_name_neg)
var_match <- var_match %>%
  left_join(root_merge_gnps, by = c("variable" = "scan1")) %>%
  left_join(standards, by = "variable")



#var_match <- read.table(file = "variable_with_name_summerR.txt", header = T)
#var_match$variable <- as.factor(var_match$variable)

height_pos <- height_pos[ , !grepl( "L" , names( height_pos ) ) ]
height_neg <- height_neg[ , !grepl( "L" , names( height_neg ) ) ]
```
...
#get the columns in the proper order
#this needs to be set to all columns with peak heights in them! Change the numbers in the line below
height_neg$max <- apply(height_neg[6:49],1,max)
...
#combine pos and neg datasets
height_merge <- rbind(height_pos_2, height_neg_2)
```

```{r}
## Load the mapping file to label the sample types
lc_map <- read.table("LCMS_map_root_summer2.txt", header = T, row.names = 1)
...
```{r}
#make a PCA plot of root metabolites
...
library(tidyr)
#set the function correctly depending on which version of dplyr you are using
nest <- nest_legacy
...
KS4_1_nd_root <-  comp.lm_results_ND_export_1 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0) 

KS4_1_d_root <-  comp.lm_results_D_export_1 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)

WT_1_nd_root <-  comp.lm_results_ND_export_1 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)

WT_1_d_root <-  comp.lm_results_D_export_1 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)

nd_1_WT_root <- comp.lm_results_WT_export_1  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)
d_1_WT_root <- comp.lm_results_WT_export_1  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)
nd_1_KS4_root <- comp.lm_results_ksl4_export_1  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)
d_1_KS4_root <- comp.lm_results_ksl4_export_1  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)
```
...
library(tidyr)
#set the functions properly depending on which version of dplyr you are using
nest <- nest_legacy
unnest <- unnest_legacy
...
KS4_2_nd_root <-  comp.lm_results_ND_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)

KS4_2_d_root <-  comp.lm_results_D_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)

WT_2_nd_root <-  comp.lm_results_ND_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)

WT_2_d_root <-  comp.lm_results_D_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)

nd_2_WT_root <- comp.lm_results_WT_export_2  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)
d_2_WT_root <- comp.lm_results_WT_export_2  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)
nd_2_KS4_root <- comp.lm_results_ksl4_export_2  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)
d_2_KS4_root <- comp.lm_results_ksl4_export_2  %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)


comp.lm_results_D_ALL <- comp.lm_results_D %>% 
  arrange(estimate) %>% 
  filter(term == "Genotypeksl4")%>% 
  mutate(significance = "2_ws_duetogeno")

comp.lm_results_ND_ALL <- comp.lm_results_ND %>% 
  arrange(estimate) %>% 
  filter(term == "Genotypeksl4")%>% 
  mutate(significance = "2_ww_duetogeno")

#write_xlsx(comp.lm_results_D_ALL, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_all_ws_duetogeno.xlsx")

#write_xlsx(comp.lm_results_ND_ALL, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_all_ww_duetogeno.xlsx")

```


#don't separate by water status
```{r}
#Calculate which peaks are significantly enriched in different sample types, timepoint 2 (V12)
#first, subset the data by different sample types and merge with the sample map
rownames(height_merge) <- height_merge[,"ID_2"]
map_sub_2 <- subset(map_sub, Timepoint == "2")
height_sub_2 <- height_merge[,match(row.names(map_sub_2), colnames(height_merge))]
height_sub_2 <-  as.data.frame(t(height_sub_2))
height_sub_2 <- tibble::rownames_to_column(height_sub_2, "SampleID")

readymelt <- inner_join(map_sub_2, height_sub_2, by = c("sample_ID_align.1" = "SampleID" ))

height_sub_2_tidy <- melt(readymelt, id.vars = names(map_sub))
str(height_sub_2_tidy)
...
#not drought only, comparing the genotypes
comp.lm <- height_sub_2_tidy %>% 
  group_by(variable) %>% 
  summarise(prev = sum(value > 0) / n()) %>% 
  filter(prev >= 0) %>% 
  inner_join(height_sub_2_tidy, by = c("variable")) %>% 
  group_by(variable) %>% 
  na.omit() %>% 
  nest() %>% 
  mutate(nb_model = map(data, ~lm(value ~ Genotype, .))) %>% 
  mutate(type = "No offset") 


comp.lm_results <- comp.lm %>%
  unnest(map(nb_model, tidy)) %>% 
  inner_join(var_match, by = "variable") %>% 
  mutate(p.adj = p.adjust(p.value, "BH"))
head(comp.lm_results)

comp.lm_results_export_2 <- comp.lm_results %>% 
  filter(p.adj <= 0.1) %>%
  arrange(estimate) %>% 
  filter(term == "Genotypeksl4")%>% 
  mutate(significance = "2_allw_duetogeno")




#make lists of enriched features for each sample type
KS4_2_allw_root <-  comp.lm_results_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate > 0)
KS4_2_allw_root <- KS4_2_allw_root[!duplicated(KS4_2_allw_root$variable),]

WT_2_allw_root <-  comp.lm_results_export_2 %>% 
  filter(p.value <= 0.1) %>% 
  filter(estimate < 0)
WT_2_allw_root <- WT_2_allw_root[!duplicated(WT_2_allw_root$variable),]



#write_xlsx(WT_2_allw_root, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_wtup_allw_duetogeno.xlsx")

#write_xlsx(KS4_2_allw_root, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_ks4up_allw_duetogeno.xlsx")
...
allsig_geno_superlist <- comp.lm_results_ND_export_1 %>%
...
allsig_water_superlist <- comp.lm_results_WT_export_1 %>%
...
WT_incommon_2 <- WT_2_d_root %>%
 # inner_join(WT_1_d_root, by = "variable")  %>%
 # inner_join(WT_2_d_root, by = "variable") %>%
  inner_join(WT_2_nd_root, by = "variable") 

head(WT_incommon_2)
WT_incommon_2 <- WT_incommon_2[!duplicated(WT_incommon_2$variable),]

ksl4_incommon_ALL <- KS4_2_d_root %>%
 inner_join(KS4_1_d_root, by = "variable")  %>%
 inner_join(KS4_1_nd_root, by = "variable") %>%
inner_join(KS4_2_nd_root, by = "variable") 

ksl4_incommon_ALL <- ksl4_incommon_ALL[!duplicated(ksl4_incommon_ALL$variable),]
```


```{r}
#make average peak intensity by sample type so that you can export the sample type average along with the peak information, for peaks commonly enriched

average_by_sampletype_R <- height_merge %>%
...
average_by_sampletype_R$samptype <- paste(average_by_sampletype_R$Genotype, average_by_sampletype_R$Water, average_by_sampletype_R$Timepoint, sep = "")



average_by_sampletype_R <- average_by_sampletype_R[,-c(2:4)]
#-----------

average_by_sampletype_R_WT2 <- WT_incommon_2 %>% 
  inner_join(average_by_sampletype_R, by = c("variable" = "ID_2")) 

head(average_by_sampletype_R_WT2)

average_by_sampletype_R_WT3 <- average_by_sampletype_R_WT2 %>% 
  spread(samptype, avg) %>% 
  mutate(log2_2d = log((ksl4d2/wtd2),2)) %>%
  mutate(log2_2nd = log((ksl4nd2/wtnd2),2))
head(average_by_sampletype_R_WT3)
  
write_xlsx(average_by_sampletype_R_WT3, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/WT_INCOMMON_FULL_2.xlsx")



#--------------
average_by_sampletype_R_KS4_all <- ksl4_incommon_ALL %>% 
  inner_join(average_by_sampletype_R, by = c("variable" = "ID_2")) 

head(average_by_sampletype_R_KS4_all)

average_by_sampletype_R_KS4_all_3 <- average_by_sampletype_R_KS4_all %>% 
  spread(samptype, avg) %>% 
  mutate(log2_2d = log((ksl4d2/wtd2),2)) %>%
  mutate(log2_2nd = log((ksl4nd2/wtnd2),2))
head(average_by_sampletype_R_KS4_all_3)
  
write_xlsx(average_by_sampletype_R_KS4_all_3, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/KS4_INCOMMON_FULL_all_ROOT.xlsx")

```

#make list for features enriched in WT samples, not separated for water status
```{r}

average_by_sampletype_R_noW <- height_merge %>%
  gather(SampleID, value, -ID_2) %>%
  mutate(ID_2 = as.character(ID_2))%>%
  mutate(value = as.numeric(value)) %>%
  inner_join(map_sub, by = c("SampleID" = "sample_ID_align.1")) %>%
  group_by(ID_2, Timepoint, Genotype)%>%
  summarize(avg = mean(value, na.rm = TRUE))

average_by_sampletype_R_noW$samptype2 <- paste(average_by_sampletype_R_noW$Genotype, average_by_sampletype_R_noW$Timepoint, sep = "")

average_by_sampletype_R_noW <- average_by_sampletype_R_noW[,-c(2:3)]

average_by_sampletype_R_WTall2 <- WT_2_allw_root %>% 
  inner_join(average_by_sampletype_R_noW, by = c("variable" = "ID_2")) 

average_by_sampletype_R_WTall3 <- average_by_sampletype_R_WTall2 %>% 
  spread(samptype2, avg) %>% 
  mutate(log2_2 = log((ksl42/wt2),2))  %>%
  filter(log2_2 <= -2.5)

head(average_by_sampletype_R_WTall3)

write_xlsx(average_by_sampletype_R_WTall3, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_wtup_allw_duetogeno_2_5fold.xlsx")
```
...
nrow(allsig_geno_superlist)
# here combine so you have one master significance list! 
allsig_geno_superlist <- allsig_geno_superlist [!duplicated(allsig_geno_superlist$variable),]

average_by_sampletype_R_genos<-  allsig_geno_superlist %>% 
  inner_join(average_by_sampletype_R, by = c("variable" = "ID_2")) 

average_by_sampletype_R_genos2 <- average_by_sampletype_R_genos %>% 
  spread(samptype, avg) %>% 
  mutate(log2_2d = log((ksl4d2/wtd2),2)) %>% 
  mutate(log2_1nd = log((ksl4nd1/wtnd1),2)) %>%
  mutate(log2_1d = log((ksl4d1/wtd1),2)) %>%
  mutate(log2_2nd = log((ksl4nd2/wtnd2),2))
head(average_by_sampletype_R_genos2)


write_xlsx(average_by_sampletype_R_genos2, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_sig_all_bygenos.xlsx")
...
allsig_water_superlist <- allsig_water_superlist [!duplicated(allsig_water_superlist$variable),]

average_by_sampletype_R_water<-  allsig_water_superlist %>% 
  inner_join(average_by_sampletype_R, by = c("variable" = "ID_2")) 

average_by_sampletype_R_water2 <- average_by_sampletype_R_water %>% 
  spread(samptype, avg) %>% 
...
head(average_by_sampletype_R_water2)
...
write_xlsx(average_by_sampletype_R_water2, "/Users/katiemurphy/Desktop/KS4_paper_2021/LCMS/results_02272022/R_sig_all_bywater.xlsx")
...
venn.diagram(
  x = list(WT_1_d_root$variable, WT_1_nd_root$variable, WT_2_d_root$variable, WT_2_nd_root$variable),
  category.names = c("WT_1_d_root" , "WT_1_nd_root", "WT_2_d_root", "WT_2_nd_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/WT.png',
  output=TRUE, 
fill = mCol4)

venn.diagram(
  x = list(KS4_1_d_root$variable, KS4_1_nd_root$variable, KS4_2_d_root$variable, KS4_2_nd_root$variable),
  category.names = c("KS4_1_d_root" , "KS4_1_nd_root", "KS4_2_d_root", "KS4_2_nd_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/KS4.png',
  output=TRUE, 
fill = mCol4)

venn.diagram(
  x = list(KS4_1_d_root$variable, KS4_1_nd_root$variable, WT_1_d_root$variable, WT_1_nd_root$variable),
  category.names = c("KS4_1_d_root" , "KS4_1_nd_root", "WT_1_d_root", "WT_1_nd_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/tp1.png',
  output=TRUE, 
fill = mCol4)

venn.diagram(
  x = list(KS4_2_d_root$variable, KS4_2_nd_root$variable, WT_2_d_root$variable, WT_2_nd_root$variable),
  category.names = c("KS4_2_d_root" , "KS4_2_nd_root", "WT_2_d_root", "WT_2_nd_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/tp2.png',
  output=TRUE, 
fill = mCol4)

venn.diagram(
  x = list(d_1_KS4_root$variable, d_2_KS4_root$variable, d_1_WT_root$variable, d_2_WT_root$variable),
  category.names = c("d_1_KS4_root" , "d_2_KS4_root", "d_1_WT_root", "d_2_WT_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/drought_on_genos.png',
  output=TRUE, 
fill = mCol4)

venn.diagram(
  x = list(nd_1_KS4_root$variable, nd_2_KS4_root$variable, nd_1_WT_root$variable, nd_2_WT_root$variable),
  category.names = c("nd_1_KS4_root" , "nd_2_KS4_root", "nd_1_WT_root", "nd_2_WT_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/nd_on_genos.png',
  output=TRUE, 
fill = mCol4)


venn.diagram(
  x = list(allsig_KS4_leaf$variable, allsig_KS4_root$variable, allsig_WT_leaf$variable, allsig_WT_root$variable),
  category.names = c("allsig_KS4_leaf" , "allsig_KS4_root", "allsig_WT_leaf", "allsig_WT_root"),
  filename = '/Users/katiemurphy/Desktop/Sept2020-selected/results/LvsR.png',
  output=TRUE, 
fill = mCol4)
...
boxplot_names <- read_excel("boxplots_03232022.xlsx")
head(boxplot_names)

WT_up_incommon_plot <- boxplot_names %>% 
  inner_join(new_intens, by = c("variable" = "ID_2")) %>% 
  group_by(variable) #%>%


plot_a <- ggplot(subset(WT_up_incommon_plot), aes(x = Genotype, y = value, fill = interaction(Water,Timepoint))) +
  geom_boxplot() +
...
root_standards_plot <- standards %>% 
  inner_join(new_intens, by = c("variable" = "ID_2")) %>% 
  group_by(variable) #%>%


root_standards_plot <- ggplot(subset(root_standards_plot), aes(x = Genotype, y = value, fill = interaction(Water,Timepoint))) +
  geom_boxplot() +
  facet_wrap( ~ variable, scales = "free", ncol = 4) +
  theme_bw() +
  labs(y = "value") +
...
quant_file_203$quant <- ((quant_file_203$value - 638114)/23163846)*3
quant_file_304$quant <- ((quant_file_304$value - 1236933)/67834117)*3
...
```{r}
#calculate statistics using anova and t-tests for the peaks in which there are standards to determine if there is significant enrichment or depletion

full_model = lm(quant ~ Genotype*Water*Timepoint, quant_file_304)
summary(full_model)
anova(full_model)

library(lsmeans)
paired = lsmeans(full_model,pairwise ~ Genotype|Water|Timepoint)$contrasts
paired

paired2 = lsmeans(full_model,pairwise ~ Water|Genotype|Timepoint)$contrasts
paired2

paired3 = lsmeans(full_model,pairwise ~ Timepoint|Genotype|Water)$contrasts
paired3

full_model = lm(quant ~ Genotype*Water*Timepoint, quant_file_203)
summary(full_model)
anova(full_model)
...
quant_file_thd <- new_intens %>%
  filter(ID_2 == "positive-9867")

full_model = lm(value ~ Genotype*Water*Timepoint, quant_file_thd)
summary(full_model)
anova(full_model)
